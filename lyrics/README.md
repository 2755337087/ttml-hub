...
- `.ttml` 保存原始歌词，不改写内容。
- 同名 `.meta.json` 保存歌曲名称、所有艺术家、多个专辑、语言、是否有翻译、是否有注音、平台 ID 和授权来源。
- 有 Apple Music、QQ 音乐或网易云 ID 时，系统据此生成稳定内部 ID；没有时生成随机 16 位 ID。
- 艺术家是数组，没有“主艺术家”，因此适合合唱、feat.、组合或不同演出版。
- Apple Music、QQ 音乐、网易云音乐任意一个同类型 ID 相同，就会被识别为同一首歌曲；歌名不参与判重，覆盖前必须确认。
- 完全没有平台 ID 的歌词会获得永久 `ttmlHubId`，并同时写入 TTML 和 `.meta.json`，以后使用该入库版本上传时仍可准确判重。
...
浏览器打开终端显示的 `http://127.0.0.1:4173`，把多个 TTML 同时拖入页面或点击多选。页面会逐个解析并列出结果，重复歌词默认跳过；如需更新已有歌词，需要单独勾选覆盖。确认后：
